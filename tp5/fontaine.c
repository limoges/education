...
void* attendre(void* data)
{
    ...
    while (1)
    {
        for (rang = 0; rang < TOTAL && p->nom != paysans[rang]->nom; rang++); // on trouve le rang
        if (rang < NB_ROBINETS) // si on est aux robinets
        {
            sleep(p->qte);
            for (rang = 0; rang < TOTAL && p->nom != paysans[rang]->nom; rang++); // on trouve le rang
            pthread_mutex_lock(&mutex);
                for (i = rang; i < nb_attente; i++)
                    paysans[i] = paysans[i+1]; // on avance chacun des paysans dans la file
                nb_attente--;
